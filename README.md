...
| 器件 | 备注 |
| --- | --- |
| STM32F407ZGT6 最小系统板 | 推荐使用正点原子开发板；若使用非正点原子板子，需要额外外接 W25Q128 SPI Flash |
...
| W25Q128 SPI Flash（16MB） | **非正点原子 F4 板子必须自备**，用于 OTA 固件存储、A/B 分区、升级标志区 |
| ST‑LINK / DAP 高速下载器 | 程序烧录调试 |
| 串口转 TTL 模块 | 日志打印、ESP8266 调试 |
| 杜邦线若干 | 硬件接线 |
> 💡提示：CAN 总线需要在总线两端增加 120Ω 终端电阻，保证 CAN 通信稳定。

## 💻 软件说明

本项目全部工程基于`STM32CubeMX + HAL库`开发，操作系统采用 FreeRTOS；图形库使用 LVGL8.x。
整体软件分为三层：
...
#define OTA_PRODUCT_ID      ""    // OneNET产品ID
#define OTA_DEVICE_NAME     ""    // 设备名称
#define OTA_USER_ID         ""    // 用户ID
#define OTA_AUTHORIZATION   ""    // 鉴权token
2.`MID/ESP_MQTT.h` MQTT 与 WiFi 配置
#define MQTT_PRODUCT_ID     ""     // OneNET产品ID
#define MQTT_DEVICE_NAME    ""     // 设备名称
#define MQTT_AUTH_PARAM     ""     // MQTT鉴权参数
#define USER_SSID           ""     // 连接WiFi名称
#define USER_PASS           ""     // WiFi密码
